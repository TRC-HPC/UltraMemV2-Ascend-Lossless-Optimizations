...
class RMSLayerNorm(LayerNormBase):
    ...
    def __init__(
        ...
        super().__init__(config, size=size, elementwise_affine=elementwise_affine)
        self.convert2fp32 = getattr(self.config, 'convert2fp32', False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.autocast(enabled=False, device_type=x.device.type):
           og_dtype = x.dtype
           new_dtype = torch.float32 if self.convert2fp32 else og_dtype
           x = x.to(new_dtype)
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.ffn_input_after_norm = None
...
class OLMo(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True, cfg=None):
        super().__init__()
        self.config = config
        ...
        def parse_memory_layer_path(config_str):
            """
            5:8/9:13/17:21
            ->
            {5: [8,0], 9: [13,1], 17: [21,2]
            """
            if config_str == 'full':
                return 'full'
            memory_layer_path = OrderedDict()
            if config_str == "no_mem":
                return memory_layer_path
            count = 0

            for conf in config_str.split('/'):
                k, v = conf.split(":")
                memory_layer_path[int(k)] = [int(v),count]
                count += 1
            return memory_layer_path

        self.mem_insert_way = parse_memory_layer_path(config.mem_insert_way)
        self.mem_layer_num = config.n_layers if self.mem_insert_way == 'full' else len(self.mem_insert_way)

        if config.mem_type == 'ultramem_v1':
            from .ultramem_layer import UltraMemLayerV1
            mem_blocks = [UltraMemLayerV1(hidden_size=config.d_model,
                                        knum=config.mem_knum,
                                        kdim=config.mem_kdim,
                                        vdim=config.mem_vdim,
                                        knn=config.mem_knn,
                                        head=config.mem_head,
                                        tucker_rank_penalty=config.mem_tucker_rank_penalty,
                                        layer_id=i,
                                        config=config)
                        for i in range(self.mem_layer_num)]
            self.transformer.update({"mem_blocks": nn.ModuleList(mem_blocks)})
        elif config.mem_type == 'ultramem_v2':
            from .ultramem_layer_v2 import UltraMemLayerV2
            mem_blocks = [UltraMemLayerV2(hidden_size=config.d_model,
                                        knum=config.mem_knum,
                                        kdim=config.mem_kdim,
                                        vdim=config.mem_vdim,
                                        pre_vdim=config.mem_pre_vdim,
                                        knn=config.mem_knn,
                                        head=config.mem_head,
                                        value_expand_time=config.mem_value_expand_time,
                                        tucker_rank_penalty=config.mem_tucker_rank_penalty,
                                        layer_id=i,
                                        has_value=False, 
                                        share_ratio=config.mem_share_ratio,
                                        mem_layer_num=self.mem_layer_num,
                                        config=config)
                        for i in range(self.mem_layer_num)]
            self.transformer.update({"mem_blocks": nn.ModuleList(mem_blocks)})
            if self.mem_layer_num > 0:
                full_memory_layer = UltraMemLayerV2(hidden_size=config.d_model,
                                            knum=config.mem_knum,
                                            kdim=config.mem_kdim,
                                            vdim=config.mem_vdim,
                                            pre_vdim=config.mem_pre_vdim,
                                            ...
                                            mem_layer_num=self.mem_layer_num,
                                            config=config)
                self.transformer.update({"full_memory_layer": full_memory_layer})
        elif config.mem_type == 'memory_plus':
            from.memory_plus_layer import MemoryLayerPlus
            mem_blocks = [MemoryLayerPlus(hidden_size=config.d_model,
                                        knum=config.mem_knum,
                                        kdim=config.mem_kdim,
                                        vdim=config.mem_vdim,
                                        knn=config.mem_knn,
                        ...
                        for i in range(self.mem_layer_num)]
            self.transformer.update({"mem_blocks": nn.ModuleList(mem_blocks)})
            full_memory_layer = MemoryLayerPlus(hidden_size=config.d_model,
                                        knum=config.mem_knum,
                                        kdim=config.mem_kdim,
                                        vdim=config.mem_vdim,
                                        knn=config.mem_knn,
            ...
            raise NotImplementedError(f'mem_type [{config.mem_type}] not implemented')
        ...
        if init_params and self.config.init_device != "meta":
            self.reset_parameters(cfg.distributed_strategy)
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            ...
            raise ValueError('not support in mem layer')
