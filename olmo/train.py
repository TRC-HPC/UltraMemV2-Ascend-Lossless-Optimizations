...
try:
    ...
    pass
from olmo.ultramem_layer import UltraMemLayerV1
from olmo.ultramem_layer_v2 import UltraMemLayerV2
from olmo.memory_parallel import get_memory_layer_data_parallel_group
...
class SpeedMonitor:
    ...
    def check(self) -> Dict[str, float]:
        if self.device_num is None:
            self.device_num = get_world_size()
        metrics: Dict[str, float] = {"throughput/total_tokens": self.global_total_tokens}
        metrics["throughput/total_tokens(B)"] = round(self.global_total_tokens/1024/1024/1024, 3)
        ...
        if self.start_times:
            ...
            metrics["throughput/tokens_per_second(M)"] = round(interval_tokens*self.device_num/interval_seconds/1024/1024, 3)
...
class Trainer:
    ...
    def load_trainer_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        new_learning_rate = self.scheduler.get_lr(
            self.cfg.optimizer.learning_rate, self.scheduler_current, self.scheduler_max, False, None, None, self.cfg.model.mem_type
        )
        for group in self.optim.param_groups:
            has_mem_value = False
            for param_name in group['param_names']:
                if 'values_for_look_up' in param_name:
                    has_mem_value = True
                    break
            if has_mem_value:
                group["initial_lr_times"] = self.cfg.optimizer.mem_value_lr_times
                group["is_mem_value"] = True
                group['mem_value_lr_max_steps_rate'] = self.cfg.optimizer.mem_value_lr_max_steps_rate
            else:
                group["initial_lr_times"] = 1.0
                group["is_mem_value"] = False
    ...
    def _save_checkpoint(
        ...
        if link_latest:
            # Link to 'latest'.
            latest_path = Path(self.cfg.save_folder) / "latest_checkpointed_iteration.txt"
            try:
                with open(latest_path, 'w') as file:
                    file.write(f'{checkpoint_dir.name}\n')
            except FileExistsError:
    ...
    def train_batch(
        ...
        for module in self.dist_model.modules():
            if isinstance(module, UltraMemLayerV1) or isinstance(module, UltraMemLayerV2):
                with FSDP.summon_full_params(self.dist_model):
                    module._online_update_tucker_approx()
        ...
        for micro_batch_idx, micro_batch in enumerate(micro_batches):
            ...
            with grad_sync_context():
                ...
                if self.should_log_optim_metrics_this_step() and self.global_step % self.cfg.wandb.log_interval == 0:
                    log_matrix = {}
                    reduce_tensor = []
                    name_list = []
                    # for name, param in sorted(list(self.model.named_parameters()), key=lambda x: x[0]):
                    #     abs_name = name.replace('transformer.blocks.', 'grad_norm/layer_')
                    #     abs_name = abs_name.replace('._fsdp_wrapped_module.', '/')
                    #     rel_name = name.replace('transformer.blocks.', 'rel_grad_norm/layer_')
                    #     rel_name = rel_name.replace('._fsdp_wrapped_module.', '/')
                    #     name_list.append(abs_name)
                    #     name_list.append(rel_name)

                    #     if param.grad is not None:
                    #         reduce_tensor.append(param.grad.norm().detach() / np.sqrt(param.grad.numel()))
                    #         reduce_tensor.append(param.grad.norm().detach() / param.norm().detach())
                    #     elif hasattr(param, "main_grad"):
                    #         reduce_tensor.append(param.main_grad.norm().detach() / np.sqrt(param.main_grad.numel()))
                    #         reduce_tensor.append(param.main_grad.norm().detach() / param.norm().detach())
                    #     else:
                    #         reduce_tensor.append(torch.tensor(0, device=self.device))
                    #         reduce_tensor.append(torch.tensor(0, device=self.device))
                    
                    # reduce_tensor = torch.stack(reduce_tensor)
                    # print('reduce_tensor',reduce_tensor)
                    # dist.reduce(reduce_tensor, dst=0, op=dist.ReduceOp.AVG)
                    # log_matrix.update(dict(zip(name_list, reduce_tensor.cpu().tolist())))

                    # memory log
                    if self.cfg.distributed_strategy==DistributedStrategy.fsdp:
                        if len(self.model.transformer.mem_blocks) > 0 and self.model.transformer.mem_blocks[0].module.output_std is not None:
                            for idx, mem in enumerate(self.model.transformer.mem_blocks):
                                indice_all = mem.module.indice_for_log
                                # print('indice_all single,',indice_all.sum())
                                # torch.cuda.synchronize()
                                # dist.all_reduce(indice_all, op=dist.ReduceOp.SUM)
                                # print('indice_all two,',indice_all.sum())
                                value_cover1 = (indice_all>0).float().sum() / mem.module.virtual_value_num

                                # 和均匀分布的kl散度
                                cur_prob = indice_all / torch.sum(indice_all)
                                uniform_gt_prob = torch.ones_like(cur_prob, device=cur_prob.device) / cur_prob.shape[0]
                                kl_value_dist_cur = F.kl_div(uniform_gt_prob.log(), cur_prob, reduction='sum')

                                output_std = mem.module.output_std
                                score_top1_mean = mem.module.score_top1_mean
                                score_topn_mean = mem.module.score_topn_mean

                                mem.module.indice_for_log = None
                                mem.module.output_std = None
                                mem.module.score_top1_mean = None
                                mem.module.score_topn_mean = None

                                reduce_tensor.append(value_cover1)
                                name_list.append(f'ultramem/value_cover/mem_layer_{idx:03d}')

                                reduce_tensor.append(kl_value_dist_cur)
                                name_list.append(f'ultramem/kl_diverse/mem_layer_{idx:03d}')

                                reduce_tensor.append(output_std)
                                name_list.append(f'ultramem/output_std/mem_layer_{idx:03d}')

                                reduce_tensor.append(score_top1_mean)
                                name_list.append(f'ultramem/score_top1_mean/mem_layer_{idx:03d}')

                                reduce_tensor.append(score_topn_mean)
                                name_list.append(f'ultramem/score_topn_mean/mem_layer_{idx:03d}')
                        if len(reduce_tensor) > 0:
                            reduce_tensor = torch.stack(reduce_tensor)
                        else:
                            reduce_tensor = None
                    elif self.cfg.distributed_strategy==DistributedStrategy.ddp:
                        if len(self.model.transformer.mem_blocks) > 0 and self.model.transformer.mem_blocks[0].output_std is not None:
                            for idx, mem in enumerate(self.model.transformer.mem_blocks):
                                indice_all = mem.indice_for_log
                                # print('indice_all single,',indice_all.sum())
                                # torch.cuda.synchronize()
                                # dist.all_reduce(indice_all, op=dist.ReduceOp.SUM)
                                # print('indice_all two,',indice_all.sum())
                                value_cover1 = (indice_all>0).float().sum() / mem.virtual_value_num
                                ...
                                output_std = mem.output_std
                                score_top1_mean = mem.score_top1_mean
                                score_topn_mean = mem.score_topn_mean

                                mem.indice_for_log = None
                                mem.output_std = None
                                mem.score_top1_mean = None
                                mem.score_topn_mean = None
                    ...
                    if reduce_tensor is not None:
                        ...
                        if wandb.run is not None and self.cfg.wandb is not None:
                            wandb.log(log_matrix, step=self.global_step)
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        for group in self.optim.param_groups:
            for name, p in zip(group["param_names"], group["params"]):
                if 'values_for_look_up' in name or 'pre_values_for_look_up' in name:
                    if not self.cfg.model.vertical_parallel:
                        dist.all_reduce(p.main_grad, op=dist.ReduceOp.SUM)
                    elif get_world_size() // self.cfg.model.mem_parallel_size > 1:
                        dist.all_reduce(p.main_grad, op=dist.ReduceOp.SUM, group=get_memory_layer_data_parallel_group())
        ...
        optim_metrics = self.optim.clip_grads_and_collect_metrics(
            ...
            mem_vertical_parallel=self.cfg.model.vertical_parallel,
        )
        ...
        for group in self.optim.param_groups:
            ...
            group["lr"] = self.scheduler.get_lr(
                ...
                group['is_mem_value'], 
                group['initial_lr_times'] if 'initial_lr_times' in group else None, 
                group['mem_value_lr_max_steps_rate'] if 'mem_value_lr_max_steps_rate' in group else None,
                self.cfg.model.mem_type,
            )
        ...
        if not self.cfg.turn_off_optimizer_log:
        ...
        if lb_batch_loss is not None:
            ...
            if not self.cfg.turn_off_moe_expert_log and expert_assignments is not None:
        ...
        with torch_profiler as p:
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in self.train_loader:
                    ...
                    if not self.cfg.dont_check_job_canceled and not cancel_initiated and self.global_step % self.cfg.canceled_check_interval == 0:
